...
type NdbClusterLister interface {
	...
	List(selector labels.Selector) (ret []*v1alpha1.NdbCluster, err error)
...
// List lists all NdbClusters in the indexer.
func (s *ndbClusterLister) List(selector labels.Selector) (ret []*v1alpha1.NdbCluster, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1alpha1.NdbCluster))
	})
	return ret, err
}
...
type NdbClusterNamespaceLister interface {
	...
	// Objects returned here must be treated as read-only.
	Get(name string) (*v1alpha1.NdbCluster, error)
...
// List lists all NdbClusters in the indexer for a given namespace.
func (s ndbClusterNamespaceLister) List(selector labels.Selector) (ret []*v1alpha1.NdbCluster, err error) {
...
// Get retrieves the NdbCluster from the indexer for a given namespace and name.
func (s ndbClusterNamespaceLister) Get(name string) (*v1alpha1.NdbCluster, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1alpha1.Resource("ndbcluster"), name)
	}
	return obj.(*v1alpha1.NdbCluster), nil
}
